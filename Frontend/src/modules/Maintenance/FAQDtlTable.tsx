...
import FAQDtlForm from "./FAQDtlForm";

export default function FAQDtlTable({
  onPageChange,
  pageProps,
  customInputs,
  onSubmit,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setLoading(true);
      const response = await FAQ.deleteFAQDTL(selectedRow.id);
  ...
  const columns = [
    ...
    { field: "label", headerName: "Title", flex: 1 },
  ...
  return (
    <>
      <Dialog open={open} onClose={handleOpenClose}>
        <DialogTitle>{selectedRow ? "Edit FAQ" : "New FAQ"}</DialogTitle>
        <DialogContent>
          <FAQDtlForm
            refetch={getDataList}
