...
export default function SectionTable({
  ...
  const handleConfirmToggle = async () => {
    ...
    try {
      ...
      const responseUpdate = await Section.updateSection({
        id: selectedRow.id,
        body: {
          b_active: updatedStatus,
          section_description: selectedRow.label,
          section_id: selectedRow.data_id,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setLoading(true);
      const response = await Section.deleteSection(selectedRow.id);
  ...
  return (
    <>
      <Dialog open={open} onClose={handleOpenClose}>
        <DialogTitle>
          {selectedRow ? "Edit Section" : "New Section"}
        </DialogTitle>
        <DialogContent>
          <SectionForm
            refetch={getDataList}
